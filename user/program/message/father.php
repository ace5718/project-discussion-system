...
foreach(explode(",", $_GET["father"]) as $message_id){
	$messages[] = select("*", "message", ["id" => $message_id]);
}
?>
...
<div class="row-fluid">
	...
	<form style="text-align:center;margin:0px;" method="post">
		<div class="btn-group">
			<a class="btn" href="?page=user/program/message/index&project_id=<?= $_GET["project_id"]?>&program_id=<?= $_GET["program_id"]?>">返回</a>
		</div>
		<br><br>
	</form>
